...
// Declare template
let indexTmpl = Template.Cement_locationTransfer,
    actionTmpl = Template.Cement_locationTransferAction,
    newTmpl = Template.Cement_locationTransferNew,
    editTmpl = Template.Cement_locationTransferEdit,
    showTmpl = Template.Cement_locationTransferShow;
...
indexTmpl.events({
    'click .js-create' (event, instance) {
        alertify.locationTransfer(fa('plus', TAPi18n.__('cement.locationTransfer.title')), renderTemplate(newTmpl)).maximize();
    },
    'click .js-update' (event, instance) {
        alertify.locationTransfer(fa('pencil', TAPi18n.__('cement.locationTransfer.title')), renderTemplate(editTmpl, this));
    },
    'click .js-destroy' (event, instance) {
        ...
        destroyAction(
            LocationTransfers,
            {_id: data._id},
            {title: TAPi18n.__('cement.locationTransfer.title'), itemTitle: data._id}
        );
    },
    'click .js-display' (event, instance) {
        Meteor.call('cement.locationTransferInfo', {_id: this._id}, function (err, result) {
            if (result) {
                console.log(result);
    ...
    'click .js-locationTransfer' (event, instance) {
        ...
        let path = FlowRouter.path("cement.locationTransferReportGen", params, queryParams);
...
editTmpl.onCreated(function () {
    this.autorun(()=> {
        this.subscribe('cement.locationTransfer', {_id: this.data._id});
    });
});
editTmpl.events({
    'click #btn-save-print'(event, instance){
...
showTmpl.helpers({
    i18nLabel(label){
        let key = `cement.locationTransfer.schema.${label}.label`;
...
AutoForm.addHooks([
    'Cement_locationTransferNew',
    'Cement_locationTransferEdit'
], hooksObject);
