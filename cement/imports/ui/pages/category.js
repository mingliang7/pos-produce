...
// Declare template
let indexTmpl = Template.Cement_category,
    actionTmpl = Template.Cement_categoryAction,
    newTmpl = Template.Cement_categoryNew,
    editTmpl = Template.Cement_categoryEdit,
    showTmpl = Template.Cement_categoryShow;
...
indexTmpl.events({
    'click .js-create' (event, instance) {
        ...
        alertify.category(fa('plus', TAPi18n.__('cement.category.title')), renderTemplate(newTmpl));
    },
    'click .js-update' (event, instance) {
        Session.set('CategoryIdSession', this._id);
        alertify.category(fa('pencil', TAPi18n.__('cement.category.title')), renderTemplate(editTmpl, this));
    },
            ...
            if (error) {
                ...
                if (result) {
                    ...
                    destroyAction(
                        Categories,
                        {_id: id},
                        {title: TAPi18n.__('cement.category.title'), itemTitle: id}
                    );
                }
            }
        });
    ...
    'click .js-display' (event, instance) {
        alertify.categoryShow(fa('eye', TAPi18n.__('cement.category.title')), renderTemplate(showTmpl, this));
    }
});
...
editTmpl.onCreated(function () {
    this.autorun(()=> {
        this.subscribe('cement.category', {_id: this.data._id});
    });
    this.categoryList = new ReactiveVar();
    let categoryId = Session.get('CategoryIdSession');
    Meteor.call('categoryList', 'Select One | No Parent', categoryId, (err, result) => {
...
showTmpl.helpers({
    i18nLabel(label){
        let i18nLabel = `cement.category.schema.${label}.label`;
        return i18nLabel;
    },
    data () {
        let data = Categories.findOne(this._id);
