...
// Declare template
let indexTmpl = Template.Cement_ringPullTransfer,
    actionTmpl = Template.Cement_ringPullTransferAction,
    newTmpl = Template.Cement_ringPullTransferNew,
    editTmpl = Template.Cement_ringPullTransferEdit,
    showTmpl = Template.Cement_ringPullTransferShow
// Local collection
...
indexTmpl.events({
    'click .js-create' (event, instance) {
        alertify.ringPullTransfer(fa('cart-arrow-down', TAPi18n.__('cement.ringPullTransfer.title')), renderTemplate(newTmpl)).maximize();
    },
    'click .js-update' (event, instance) {
        ...
        //     Meteor.call('cement.isGroupRingPullTransferClosed', {_id: this.paymentGroupId}, (err, result)=> {
        //         if (result.paid) {
    ...
    'click .js-destroy' (event, instance) {
        ...
        destroyAction(
            RingPullTransfers,
            {_id: data._id},
            {title: TAPi18n.__('cement.ringPullTransfer.title'), itemTitle: data._id}
        );
    },
    'click .js-display' (event, instance) {
        ...
        Meteor.call('ringPullTransferShowItems', {_id: this._id}, function (err, result) {
            swal.close();
            alertify.ringPullTransferShow(fa('eye', TAPi18n.__('cement.ringPullTransfer.title')), renderTemplate(showTmpl, result)).maximize();
        });
    },
    'click .js-ringPullTransfer' (event, instance) {
        let params = {};
        let queryParams = {ringPullTransferId: this._id};
        let path = FlowRouter.path("cement.ringPullTransferReportGen", params, queryParams);
...
showTmpl.helpers({
    ...
    i18nLabel(label){
        let key = `cement.ringPullTransfer.schema.${label}.label`;
...
function excuteEditForm(doc) {
    ...
    alertify.ringPullTransfer(fa('pencil', TAPi18n.__('cement.ringPullTransfer.title')), renderTemplate(editTmpl, doc)).maximize();
}
// Hook
let hooksObject = {
    before: {
...
AutoForm.addHooks([
    'Cement_ringPullTransferNew',
    'Cement_ringPullTransferUpdate'
], hooksObject);
