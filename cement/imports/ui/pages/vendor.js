...
// Declare template
let indexTmpl = Template.Cement_vendor,
    actionTmpl = Template.Cement_vendorAction,
    newTmpl = Template.Cement_vendorNew,
    newTmplDropDown = Template.Cement_vendorNewDropDownForm,
    editTmpl = Template.Cement_vendorEdit,
    showTmpl = Template.Cement_vendorShow,
    buttonActiomTmpl = Template.Cement_vendorButtonAction;
...
indexTmpl.onCreated(function () {
    ...
    this.filter = new ReactiveTable.Filter('cement.vendorByBranchFilter', ['branchId']);
    this.autorun(()=> {
...
indexTmpl.onDestroyed(()=> {
    ReactiveTable.clearFilters(['cement.vendorByBranchFilter']);
})
...
indexTmpl.helpers({
    ...
    tableSettings(){
        let i18nPrefix = 'cement.vendor.schema';

        reactiveTableSettings.collection = 'cement.reactiveTable.vendor';
...
indexTmpl.events({
    'click .js-create' (event, instance) {
        alertify.vendor(fa('plus', TAPi18n.__('cement.vendor.title')), renderTemplate(newTmpl));
    },
    'click .js-update' (event, instance) {
        alertify.vendor(fa('pencil', TAPi18n.__('cement.vendor.title')), renderTemplate(editTmpl, this));
    },
            ...
            if (error) {
                ...
                if (result) {
                    ...
                    destroyAction(
                        Vendors,
                        {_id: id},
                        {title: TAPi18n.__('cement.vendor.title'), itemTitle: id}
                    );
                }
            }
        });
    ...
    'click .js-display' (event, instance) {
        alertify.vendorShow(fa('eye', TAPi18n.__('cement.vendor.title')), renderTemplate(showTmpl, this));
    },
    'click .go-to-pay-bill'(event, instance){
        FlowRouter.go('cement.payBill', {vendorId: this._id});
    }
});
...
editTmpl.onCreated(function () {
    ...
    this.autorun(()=> {
        this.subscribe('cement.vendor', {_id: this.data._id});
    });
});
editTmpl.events({
    'change [name="paymentType"]'(event, instance){
