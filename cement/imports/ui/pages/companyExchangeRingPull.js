...
// Declare template
let indexTmpl = Template.Cement_companyExchangeRingPull,
    actionTmpl = Template.Cement_companyExchangeRingPullAction,
    newTmpl = Template.Cement_companyExchangeRingPullNew,
    editTmpl = Template.Cement_companyExchangeRingPullEdit,
    showTmpl = Template.Cement_companyExchangeRingPullShow;
// Local collection
...
indexTmpl.events({
            ...
            if (error) {
                ...
                if (result) {
                    ...
                    destroyAction(
                        ...
                        {_id: id},
                        {title: TAPi18n.__('cement.companyExchangeRingPull.title'), itemTitle: id}
                    );
                }
            }
        });
    ...
    'click .js-display' (event, instance) {
        ...
        Meteor.call('companyExchangeRingPullShowItem', {_id: this._id}, function (err, result) {
            swal.close();
            alertify.companyExchangeRingPullShow(fa('eye', TAPi18n.__('cement.companyExchangeRingPull.title')), renderTemplate(showTmpl, result));
        });
    },
    'click .js-companyExchangeRingPull' (event, instance) {
        let params = {};
        let queryParams = {companyExchangeRingPullId: this._id};
        let path = FlowRouter.path("cement.companyExchangeRingPullReportGen", params, queryParams);
...
function excuteEditForm(doc) {
    ...
    alertify.companyExchangeRingPull(fa('pencil', TAPi18n.__('cement.companyExchangeRingPull.title')), renderTemplate(editTmpl, doc)).maximize();
}
// Hook
let hooksObject = {
    before: {
...
AutoForm.addHooks([
    'Cement_companyExchangeRingPullNew',
    'Cement_companyExchangeRingPullUpdate'
], hooksObject);
