...
// Declare template
let indexTmpl = Template.Cement_item,
	actionTmpl = Template.Cement_itemAction,
	newTmpl = Template.Cement_itemNew,
	editTmpl = Template.Cement_itemEdit,
	showTmpl = Template.Cement_itemShow,
...
indexTmpl.events({
	'click .js-create' (event, instance) {
		alertify.item(fa('plus', TAPi18n.__('cement.item.title')), renderTemplate(newTmpl)).maximize();
	},
	'click .js-update' (event, instance) {
		alertify.item(fa('pencil', TAPi18n.__('cement.item.title')), renderTemplate(editTmpl, this)).maximize();
	},
			...
			if (error) {
				...
				if (result) {
					...
					destroyAction(
						Item,
						{_id: id},
						{title: TAPi18n.__('cement.item.title'), itemTitle: id}
					);
				}
			}
		});
	...
	'click .js-display' (event, instance) {
		alertify.item(fa('eye', TAPi18n.__('cement.item.title')), renderTemplate(showTmpl, this));
	}
});
...
newTmpl.events({
	'click [name="unitId"]'(event, instance){
		alertify.addOn(fa('plus', 'Add Unit'), renderTemplate(Template.Cement_unitNew));
	},
...
showTmpl.onCreated(function () {
	...
	this.autorun(()=> {
		this.subscribe('cement.item', {_id: self._id});
	...
	tableSettings: function () {
		...
		reactiveTableSettings.fields = [
			...
			},
			{
				...
				tmpl: Template.Cement_schemeItemsAction, sortable: false
			}
		];
...
Template.schemeItem.events({
	...
	'click .js-update-item': function (event, instance) {
		alertify.tmpItem(fa('pencil', TAPi18n.__('cement.order.schema.itemId.label')), renderTemplate(Template.Cement_schemeItemsEdit, this));
	},
	'click .js-destroy-item': function (event, instance) {
		destroyAction(
			tmpCollection,
			{_id: this._id},
			{title: TAPi18n.__('cement.order.schema.itemId.label'), itemTitle: this.itemId}
		);
	}
});
//edit custom object
