...
// Declare template
let indexTmpl = Template.Cement_prepaidOrder,
    actionTmpl = Template.Cement_prepaidOrderAction,
    newTmpl = Template.Cement_prepaidOrderNew,
    editTmpl = Template.Cement_prepaidOrderEdit,
    showTmpl = Template.Cement_prepaidOrderShow;
...
indexTmpl.events({
    'click .js-create' (event, instance) {
        alertify.prepaidOrder(fa('plus', TAPi18n.__('cement.prepaidOrder.title')), renderTemplate(newTmpl)).maximize();
    },
        ...
        Meteor.call('isPrepaidOrderHasRelation', this._id, function (error, result) {
            if (error) {
                ...
                if (result) {
                    ...
                    alertify.prepaidOrder(fa('pencil', TAPi18n.__('cement.prepaidOrder.title')), renderTemplate(editTmpl, this));
                }
            }
        });
    },
            ...
            if (error) {
                ...
                if (result) {
                    ...
                    destroyAction(
                        PrepaidOrders,
                        {_id: id},
                        {title: TAPi18n.__('cement.prepaidOrder.title'), itemTitle: id}
                    );
                }
            }
        });
    ...
    'click .js-display' (event, instance) {
        Meteor.call("prepaidOrderShow", {_id: this._id}, function (err, result) {
            if (result) {
                alertify.prepaidOrderShow(fa('eye', TAPi18n.__('cement.prepaidOrder.title')), renderTemplate(showTmpl, result));
            }
        });
    },
    'click .js-invoice' (event, instance) {
        ...
        let path = FlowRouter.path("cement.prepaidOrderReportGen", params, queryParams);
...
editTmpl.onCreated(function () {
    this.autorun(()=> {
        this.subscribe('cement.prepaidOrder', {_id: this.data._id});
    });
});
...
showTmpl.helpers({
    ...
    i18nLabel(label){
        let key = `cement.prepaidOrder.schema.${label}.label`;
...
AutoForm.addHooks([
    'Cement_prepaidOrderNew',
    'Cement_prepaidOrderEdit'
], hooksObject);
