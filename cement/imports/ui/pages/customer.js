...
// Declare template
let indexTmpl = Template.Cement_customer,
    actionTmpl = Template.Cement_customerAction,
    buttonActionTmpl = Template.Cement_customerButtonAction,
    newTmpl = Template.Cement_customerNew,
    newTmplDropDownForm = Template.Cement_customerNewDropDownForm,
    editTmpl = Template.Cement_customerEdit,
    showTmpl = Template.Cement_customerShow;
...
indexTmpl.onCreated(function () {
    ...
    this.filter = new ReactiveTable.Filter('cement.customerByBranchFilter', ['branchId']);
    this.autorun(()=> {
...
indexTmpl.onDestroyed(()=> {
    ReactiveTable.clearFilters(['cement.customerByBranchFilter']);
...
indexTmpl.helpers({
    ...
    tableSettings(){
        let i18nPrefix = 'cement.customer.schema';

        reactiveTableSettings.collection = 'cement.reactiveTable.customer';
        reactiveTableSettings.filters = ['cement.customerByBranchFilter'];
...
indexTmpl.events({
    'click .js-create' (event, instance) {
        alertify.customer(fa('plus', TAPi18n.__('cement.customer.title')), renderTemplate(newTmpl));
    },
    'click .js-update' (event, instance) {
        alertify.customer(fa('pencil', TAPi18n.__('cement.customer.title')), renderTemplate(editTmpl, this));
    },
            ...
            if (error) {
                ...
                if (result) {
                    ...
                    destroyAction(
                        Customers,
                        {_id: id},
                        {title: TAPi18n.__('cement.customer.title'), itemTitle: id}
                    );
                }
            }
        });
    ...
    'click .js-display' (event, instance) {
        alertify.customerShow(fa('eye', TAPi18n.__('cement.customer.title')), renderTemplate(showTmpl, this));
    },
    'click .go-to-receive-payment'(event, instance){
        FlowRouter.go('cement.receivePayment', {customerId: this._id});
    }
});
...
editTmpl.onCreated(function () {
    ...
    this.autorun(()=> {
        this.subscribe('cement.customer', {_id: this.data._id});
    });
});
editTmpl.events({
    'change [name="paymentType"]'(event, instance){
