...
//methods
let indexTmpl = Template.Cement_ringPullRequest,
...
indexTmpl.onCreated(function () {
    ...
    this.autorun(function () {
        if (Session.get('currentBranch') || transferState.get() || statusState.get()) {
            ...
            let subscription = Meteor.subscribe('cement.activeRingPullTransfers',
                {
...
indexTmpl.events({
    ...
    'click .show-detail'(event, instance){
        Meteor.call('cement.ringPullTransfersInfo', {_id: this._id}, function (err, result) {
            if (result) {
                console.log(result);
