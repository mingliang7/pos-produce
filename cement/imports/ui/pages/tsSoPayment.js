...
let indexTmpl = Template.Cement_tsSoPayment,
    showInvoiceTmpl = Template.Cement_invoiceShowInTsSoPayment;
    ...
    this.autorun(function () {
        if (Session.get('customerId')) {
            ...
            if (customer && customer.termId) {
                invoiceSub = Meteor.subscribe('cement.activeTsSo', {
                    customerId: Session.get('customerId'),
                    $or: [{tsStatus: {$in: ['active', 'partial']}}, {tsStatus: {$exists: false}}],
                    totalTransportFee: {$ne: 0},
                });
            } else {
                invoiceSub = Meteor.subscribe('cement.activeGroupSales', {
        ...
        if (Session.get('invoices')) {
            Meteor.subscribe('cement.tsSoPayment', {
                invoiceId: {
