...
// Declare template
let indexTmpl = Template.Cement_exchangeRingPull,
    actionTmpl = Template.Cement_exchangeRingPullAction,
    newTmpl = Template.Cement_exchangeRingPullNew,
    editTmpl = Template.Cement_exchangeRingPullEdit,
    showTmpl = Template.Cement_exchangeRingPullShow;
// Local collection
...
indexTmpl.events({
    'click .js-create' (event, instance) {
        alertify.exchangeRingPull(fa('cart-arrow-down', TAPi18n.__('cement.exchangeRingPull.title')), renderTemplate(newTmpl)).maximize();
    },
    'click .js-update' (event, instance) {
        ...
        //     Meteor.call('cement.isGroupExchangeRingPullClosed', {_id: this.paymentGroupId}, (err, result)=> {
        //         if (result.paid) {
    ...
    'click .js-destroy' (event, instance) {
        ...
        destroyAction(
            ExchangeRingPulls,
            {_id: data._id},
            {title: TAPi18n.__('cement.exchangeRingPull.title'), itemTitle: data._id}
        );
    },
    'click .js-display' (event, instance) {
        ...
        Meteor.call('exchangeRingPullShow', {_id: this._id}, function (err, result) {
            swal.close();
            alertify.exchangeRingPullShow(fa('eye', TAPi18n.__('cement.exchangeRingPull.title')), renderTemplate(showTmpl, result)).maximize();
        });
    },
    'click .js-exchangeRingPull' (event, instance) {
        let params = {};
        let queryParams = {exchangeRingPullId: this._id};
        let path = FlowRouter.path("cement.exchangeRingPullReportGen", params, queryParams);
...
function excuteEditForm(doc) {
    ...
    alertify.exchangeRingPull(fa('pencil', TAPi18n.__('cement.exchangeRingPull.title')), renderTemplate(editTmpl, doc)).maximize();
}
// Hook
let hooksObject = {
    before: {
...
AutoForm.addHooks([
    'Cement_exchangeRingPullNew',
    'Cement_exchangeRingPullUpdate'
], hooksObject);
