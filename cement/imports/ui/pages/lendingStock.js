...
// Declare template
let indexTmpl = Template.Cement_lendingStock,
    actionTmpl = Template.Cement_lendingStockAction,
    newTmpl = Template.Cement_lendingStockNew,
    editTmpl = Template.Cement_lendingStockEdit,
    showTmpl = Template.Cement_lendingStockShow;
// Local collection
...
indexTmpl.events({
    'click .js-create' (event, instance) {
        alertify.lendingStock(fa('plus', TAPi18n.__('cement.lendingStock.title')), renderTemplate(newTmpl)).maximize();
    },
        ...
        Meteor.call('isLendingStockHasRelation', data._id, function (error, result) {
            if (error) {
                ...
                if (result) {
                    ...
                    alertify.lendingStock(fa('pencil', TAPi18n.__('cement.lendingStock.title')), renderTemplate(editTmpl, data));
            ...
            if (error) {
                ...
                if (result) {
                    ...
                    destroyAction(
                        LendingStocks,
                        {_id: id},
                        {title: TAPi18n.__('cement.lendingStock.title'), itemTitle: id}
                    );
                }
            }
        });
    ...
    'click .js-display' (event, instance) {
        Meteor.call('lendingStockShow', {_id: this._id}, function (err, result) {
            if (result) {
                alertify.lendingStockShow(fa('eye', TAPi18n.__('cement.lendingStock.title')), renderTemplate(showTmpl, result));
            }
        });
    },
    'click .js-lendingStock' (event, instance) {
        ...
        let path = FlowRouter.path("cement.lendingStockReportGen", params, queryParams);
...
AutoForm.addHooks([
    'Cement_lendingStockNew',
    'Cement_lendingStockEdit'
], hooksObject);
