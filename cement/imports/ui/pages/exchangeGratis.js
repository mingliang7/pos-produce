...
// Declare template
let indexTmpl = Template.Cement_exchangeGratis,
    actionTmpl = Template.Cement_exchangeGratisAction,
    newTmpl = Template.Cement_exchangeGratisNew,
    editTmpl = Template.Cement_exchangeGratisEdit,
    showTmpl = Template.Cement_exchangeGratisShow;
...
indexTmpl.events({
    'click .js-create' (event, instance) {
        alertify.exchangeGratis(fa('plus', TAPi18n.__('cement.exchangeGratis.title')), renderTemplate(newTmpl)).maximize();
    },
        ...
        Meteor.call('isExchangeGratisHasRelation', data._id, function (error, result) {
            if (error) {
                ...
                if (result) {
                    ...
                    alertify.exchangeGratis(fa('pencil', TAPi18n.__('cement.exchangeGratis.title')), renderTemplate(editTmpl, data));
                }
            }
        });
    },
            ...
            if (error) {
                ...
                if (result) {
                    ...
                    destroyAction(
                        ExchangeGratis,
                        {_id: id},
                        {title: TAPi18n.__('cement.exchangeGratis.title'), itemTitle: id}
                    );
                }
            }
        });
    ...
    'click .js-display' (event, instance) {
        ...
        Meteor.call('exchangeGratisShowItem', {_id: this._id}, function (err, result) {
            ...
            alertify.exchangeGratisShow(fa('eye', TAPi18n.__('cement.exchangeGratis.title')), renderTemplate(showTmpl, result));
        });
    },
    'click .js-invoice' (event, instance) {
        let params = {};
        let queryParams = {exchangeGratisId: this._id};
        let path = FlowRouter.path("cement.exchangeGratisReportGen", params, queryParams);
...
editTmpl.onCreated(function () {
    this.autorun(()=> {
        this.subscribe('cement.exchangeGratis', {_id: this.data._id});
    });
});
...
showTmpl.helpers({
    i18nLabel(label){
        let key = `cement.exchangeGratis.schema.${label}.label`;
...
AutoForm.addHooks([
    'Cement_exchangeGratisNew',
    'Cement_exchangeGratisEdit'
], hooksObject);
