...
TSSOPayment.before.insert(function (userId, doc) {
    doc._id = idGenerator.genWithPrefix(TSSOPayment, `${doc.branchId}-`, 9);
});
...
// });
//
TSSOPayment.after.remove(function (userId, doc) {
    Meteor.call('insertRemovedTsSoPayment', doc);
    Meteor.defer(function () {
        //Account Integration
        let setting = AccountIntegrationSetting.findOne();
        if (setting && setting.integrate) {
            let data = {_id: doc._id, type: 'TSSOPayment'};
