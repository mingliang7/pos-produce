...
let indexTmpl = Template.Cement_headerMenu;
indexTmpl.onCreated(function () {
    this.autorun(function () {
        if (Session.get('currentBranch')) {
            let branch = Meteor.subscribe('core.branch');
            let locationTransferSubscription = Meteor.subscribe('cement.activeLocationTransfers',
                {
                    toBranchId: Session.get('currentBranch'),
                    pending: true,
                    status: 'active'
                });
            let ringPullTransferSubscription = Meteor.subscribe('cement.activeRingPullTransfers', {
                toBranchId: Session.get('currentBranch'),
                pending: true,
                status: 'active'
            });
            let TransferMoneySubscription = Meteor.subscribe('cement.activeTransferMoney', {
                toBranchId: Session.get('currentBranch'),
                pending: true,
                status: 'active'
            });
