...
let indexTmpl = Template.PPOS_headerMenu;
indexTmpl.onCreated(function () {
    this.autorun(function () {
        if (Session.get('currentBranch')) {
            let branch = Meteor.subscribe('core.branch');
            let locationTransferSubscription = Meteor.subscribe('ppos.activeLocationTransfers',
                {
                    toBranchId: Session.get('currentBranch'),
                    pending: true,
                    status: 'active'
                });
            let ringPullTransferSubscription = Meteor.subscribe('ppos.activeRingPullTransfers', {
                toBranchId: Session.get('currentBranch'),
                pending: true,
                status: 'active'
            });
            let TransferMoneySubscription = Meteor.subscribe('ppos.activeTransferMoney', {
                toBranchId: Session.get('currentBranch'),
                pending: true,
                status: 'active'
            });
