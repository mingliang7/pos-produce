...
// Declare template
let indexTmpl = Template.PPOS_category,
    actionTmpl = Template.PPOS_categoryAction,
    newTmpl = Template.PPOS_categoryNew,
    editTmpl = Template.PPOS_categoryEdit,
    showTmpl = Template.PPOS_categoryShow;
...
indexTmpl.events({
    'click .js-create' (event, instance) {
        ...
        alertify.category(fa('plus', TAPi18n.__('ppos.category.title')), renderTemplate(newTmpl));
    },
    'click .js-update' (event, instance) {
        Session.set('CategoryIdSession', this._id);
        alertify.category(fa('pencil', TAPi18n.__('ppos.category.title')), renderTemplate(editTmpl, this));
    },
            ...
            if (error) {
                ...
                if (result) {
                    ...
                    destroyAction(
                        Categories,
                        {_id: id},
                        {title: TAPi18n.__('ppos.category.title'), itemTitle: id}
                    );
                }
            }
        });
    ...
    'click .js-display' (event, instance) {
        alertify.categoryShow(fa('eye', TAPi18n.__('ppos.category.title')), renderTemplate(showTmpl, this));
    }
});
...
editTmpl.onCreated(function () {
    this.autorun(()=> {
        this.subscribe('ppos.category', {_id: this.data._id});
    });
    this.categoryList = new ReactiveVar();
    let categoryId = Session.get('CategoryIdSession');
    Meteor.call('categoryList', 'Select One | No Parent', categoryId, (err, result) => {
