...
var itemsTmpl = Template.PPOS_exchangeRingPullItems,
    actionItemsTmpl = Template.PPOS_exchangeRingPullItemsAction,
    editItemsTmpl = Template.PPOS_exchangeRingPullItemsEdit;
...
itemsTmpl.helpers({
    ...
    tableSettings: function () {
        let i18nPrefix = 'ppos.exchangeRingPull.schema';
...
itemsTmpl.events({
    ...
    'click .js-update-item': function (event, instance) {
        alertify.item(fa('pencil', TAPi18n.__('ppos.exchangeRingPull.schema.itemId.label')), renderTemplate(editItemsTmpl, this));
    },
    'click .js-destroy-item': function (event, instance) {
        event.preventDefault();
        let itemDoc = this;
        if (AutoForm.getFormId() == "PPOS_exchangeRingPullUpdate") { //check if update form
            swal({
                title: "Are you sure?",
                text: "លុបទំនិញមួយនេះ?",
            ...
            destroyAction(
                itemsCollection, {
                    ...
                    title: TAPi18n.__('ppos.exchangeRingPull.schema.itemId.label'),
...
AutoForm.addHooks(['PPOS_exchangeRingPullItemsEdit'], hooksObject);
