...
// Declare template
let indexTmpl = Template.PPOS_exchangeGratis,
    actionTmpl = Template.PPOS_exchangeGratisAction,
    newTmpl = Template.PPOS_exchangeGratisNew,
    editTmpl = Template.PPOS_exchangeGratisEdit,
    showTmpl = Template.PPOS_exchangeGratisShow;
...
indexTmpl.events({
    'click .js-create' (event, instance) {
        alertify.exchangeGratis(fa('plus', TAPi18n.__('ppos.exchangeGratis.title')), renderTemplate(newTmpl)).maximize();
    },
        ...
        Meteor.call('isExchangeGratisHasRelation', data._id, function (error, result) {
            if (error) {
                ...
                if (result) {
                    ...
                    alertify.exchangeGratis(fa('pencil', TAPi18n.__('ppos.exchangeGratis.title')), renderTemplate(editTmpl, data));
                }
            }
        });
    },
            ...
            if (error) {
                ...
                if (result) {
                    ...
                    destroyAction(
                        ExchangeGratis,
                        {_id: id},
                        {title: TAPi18n.__('ppos.exchangeGratis.title'), itemTitle: id}
                    );
                }
            }
        });
    ...
    'click .js-display' (event, instance) {
        ...
        Meteor.call('exchangeGratisShowItem', {_id: this._id}, function (err, result) {
            ...
            alertify.exchangeGratisShow(fa('eye', TAPi18n.__('ppos.exchangeGratis.title')), renderTemplate(showTmpl, result));
        });
    },
    'click .js-invoice' (event, instance) {
        let params = {};
        let queryParams = {exchangeGratisId: this._id};
        let path = FlowRouter.path("ppos.exchangeGratisReportGen", params, queryParams);
...
editTmpl.onCreated(function () {
    this.autorun(()=> {
        this.subscribe('ppos.exchangeGratis', {_id: this.data._id});
    });
});
...
AutoForm.addHooks([
    'PPOS_exchangeGratisNew',
    'PPOS_exchangeGratisEdit'
], hooksObject);
