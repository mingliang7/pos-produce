...
Tracker.autorun(function () {
    ...
    if (Session.get('prepaidOrderItems')
        ...
        Meteor.subscribe('ppos.item', {_id: {$in: data}});
    }

});
// Declare template
let indexTmpl = Template.PPOS_receiveItem,
    actionTmpl = Template.PPOS_receiveItemAction,
    newTmpl = Template.PPOS_receiveItemNew,
    editTmpl = Template.PPOS_receiveItemEdit,
    showTmpl = Template.PPOS_receiveItemShow,
...
indexTmpl.events({
    'click .js-create' (event, instance) {
        alertify.receiveItem(fa('plus', TAPi18n.__('ppos.receiveItem.title')), renderTemplate(newTmpl)).maximize();
    },
    'click .js-update' (event, instance) {
        itemsCollection.remove({});
        alertify.receiveItem(fa('pencil', TAPi18n.__('ppos.receiveItem.title')), renderTemplate(editTmpl, this));
    },
    'click .js-destroy' (event, instance) {
        ...
        destroyAction(
            ReceiveItems,
            {_id: data._id},
            {title: TAPi18n.__('ppos.receiveItem.title'), itemTitle: data._id}
        );
    },
    'click .js-display' (event, instance) {
        ...
        Meteor.call('billShowItems', {doc: this}, function (err, result) {
            swal.close();
            alertify.receiveItemShow(fa('eye', TAPi18n.__('ppos.invoice.title')), renderTemplate(showTmpl, result)).maximize();
        });
    },
    'click .js-receiveItem' (event, instance) {
        let params = {};
        let queryParams = {receiveItemId: this._id};
        let path = FlowRouter.path("ppos.receiveItemReportGen", params, queryParams);
