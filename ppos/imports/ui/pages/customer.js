...
// Declare template
let indexTmpl = Template.PPOS_customer,
    actionTmpl = Template.PPOS_customerAction,
    buttonActionTmpl = Template.PPOS_customerButtonAction,
    newTmpl = Template.PPOS_customerNew,
    newTmplDropDownForm = Template.PPOS_customerNewDropDownForm,
    editTmpl = Template.PPOS_customerEdit,
    showTmpl = Template.PPOS_customerShow;
...
indexTmpl.onCreated(function () {
    ...
    this.filter = new ReactiveTable.Filter('ppos.customerByBranchFilter', ['branchId']);
    this.autorun(()=> {
...
indexTmpl.onDestroyed(()=> {
    ReactiveTable.clearFilters(['ppos.customerByBranchFilter']);
...
indexTmpl.helpers({
    ...
    tableSettings(){
        let i18nPrefix = 'ppos.customer.schema';

        reactiveTableSettings.collection = 'ppos.reactiveTable.customer';
        reactiveTableSettings.filters = ['ppos.customerByBranchFilter'];
...
indexTmpl.events({
    'click .js-create' (event, instance) {
        alertify.customer(fa('plus', TAPi18n.__('ppos.customer.title')), renderTemplate(newTmpl));
    },
    'click .js-update' (event, instance) {
        alertify.customer(fa('pencil', TAPi18n.__('ppos.customer.title')), renderTemplate(editTmpl, this));
    },
            ...
            if (error) {
                ...
                if (result) {
                    ...
                    destroyAction(
                        Customers,
                        {_id: id},
                        {title: TAPi18n.__('ppos.customer.title'), itemTitle: id}
                    );
                }
            }
        });
    ...
    'click .js-display' (event, instance) {
        alertify.customerShow(fa('eye', TAPi18n.__('ppos.customer.title')), renderTemplate(showTmpl, this));
    },
    'click .go-to-receive-payment'(event, instance){
        FlowRouter.go('ppos.receivePayment', {customerId: this._id});
    }
});
...
editTmpl.onCreated(function () {
    ...
    this.autorun(()=> {
        this.subscribe('ppos.customer', {_id: this.data._id});
    });
});
editTmpl.events({
    'change [name="paymentType"]'(event, instance){
...
showTmpl.helpers({
    i18nLabel(label){
        let i18nLabel = `cement.customer.schema.${label}.label`;
        return i18nLabel;
    },
    data () {
        let data = Customers.findOne(this._id);
...
//receive payment
Template.PPOS_customerButtonAction.helpers({
    checkIfInvoiced(){
