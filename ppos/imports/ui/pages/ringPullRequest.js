...
//methods
let indexTmpl = Template.PPOS_ringPullRequest,
...
indexTmpl.onCreated(function () {
    ...
    this.autorun(function () {
        if (Session.get('currentBranch') || transferState.get() || statusState.get()) {
            ...
            let subscription = Meteor.subscribe('ppos.activeRingPullTransfers',
                {
...
indexTmpl.events({
    ...
    'click .show-detail'(event, instance){
        Meteor.call('ppos.ringPullTransfersInfo', {_id: this._id}, function (err, result) {
            if (result) {
                console.log(result);
