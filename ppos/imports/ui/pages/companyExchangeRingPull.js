...
// Declare template
let indexTmpl = Template.PPOS_companyExchangeRingPull,
    actionTmpl = Template.PPOS_companyExchangeRingPullAction,
    newTmpl = Template.PPOS_companyExchangeRingPullNew,
    editTmpl = Template.PPOS_companyExchangeRingPullEdit,
    showTmpl = Template.PPOS_companyExchangeRingPullShow;
// Local collection
...
indexTmpl.events({
    'click .js-create' (event, instance) {
        alertify.companyExchangeRingPull(fa('cart-arrow-down', TAPi18n.__('ppos.companyExchangeRingPull.title')), renderTemplate(newTmpl)).maximize();
    },
            ...
            if (error) {
                ...
                if (result) {
                    ...
                    destroyAction(
                        ...
                        {_id: id},
                        {title: TAPi18n.__('ppos.companyExchangeRingPull.title'), itemTitle: id}
                    );
                }
            }
        });
    ...
    'click .js-display' (event, instance) {
        ...
        Meteor.call('companyExchangeRingPullShowItem', {_id: this._id}, function (err, result) {
            swal.close();
            alertify.companyExchangeRingPullShow(fa('eye', TAPi18n.__('ppos.companyExchangeRingPull.title')), renderTemplate(showTmpl, result));
        });
    },
    'click .js-companyExchangeRingPull' (event, instance) {
        let params = {};
        let queryParams = {companyExchangeRingPullId: this._id};
        let path = FlowRouter.path("ppos.companyExchangeRingPullReportGen", params, queryParams);
...
showTmpl.helpers({
    ...
    i18nLabel(label){
        let key = `cement.companyExchangeRingPull.schema.${label}.label`;
...
function excuteEditForm(doc) {
    ...
    alertify.companyExchangeRingPull(fa('pencil', TAPi18n.__('ppos.companyExchangeRingPull.title')), renderTemplate(editTmpl, doc)).maximize();
}
// Hook
let hooksObject = {
    before: {
...
AutoForm.addHooks([
    'PPOS_companyExchangeRingPullNew',
    'PPOS_companyExchangeRingPullUpdate'
], hooksObject);
