...
let indexTmpl = Template.PPOS_payBill;
...
indexTmpl.onCreated(function () {
    ...
    this.autorun(function () {
        Meteor.subscribe('ppos.advanceDiscount', {name: 'payBill'});
        if (Session.get('vendorIdState')) {
            ...
            if (vendor && vendor.termId) {
                billSub = Meteor.subscribe('ppos.activeEnterBills', {
                    vendorId: Session.get('vendorIdState'),
                    status: {$in: ['active', 'partial']},
                    billType: 'term'
                });
            } else {
                billSub = Meteor.subscribe('ppos.activeGroupBills', {
        ...
        if (Session.get('invoices')) {
            Meteor.subscribe('ppos.payBills', {
                billId: {
