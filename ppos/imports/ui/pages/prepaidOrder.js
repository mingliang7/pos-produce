...
// Declare template
let indexTmpl = Template.PPOS_prepaidOrder,
    actionTmpl = Template.PPOS_prepaidOrderAction,
    newTmpl = Template.PPOS_prepaidOrderNew,
    editTmpl = Template.PPOS_prepaidOrderEdit,
    showTmpl = Template.PPOS_prepaidOrderShow;
...
indexTmpl.events({
    'click .js-create' (event, instance) {
        alertify.prepaidOrder(fa('plus', TAPi18n.__('ppos.prepaidOrder.title')), renderTemplate(newTmpl)).maximize();
    },
        ...
        Meteor.call('isPrepaidOrderHasRelation', this._id, function (error, result) {
            if (error) {
                ...
                if (result) {
                    ...
                    alertify.prepaidOrder(fa('pencil', TAPi18n.__('ppos.prepaidOrder.title')), renderTemplate(editTmpl, this));
                }
            }
        });
    },
            ...
            if (error) {
                ...
                if (result) {
                    ...
                    destroyAction(
                        PrepaidOrders,
                        {_id: id},
                        {title: TAPi18n.__('ppos.prepaidOrder.title'), itemTitle: id}
                    );
                }
            }
        });
    ...
    'click .js-display' (event, instance) {
        Meteor.call("prepaidOrderShow", {_id: this._id}, function (err, result) {
            if (result) {
                alertify.prepaidOrderShow(fa('eye', TAPi18n.__('ppos.prepaidOrder.title')), renderTemplate(showTmpl, result));
            }
        });
    },
    'click .js-invoice' (event, instance) {
        ...
        let path = FlowRouter.path("ppos.prepaidOrderReportGen", params, queryParams);
...
editTmpl.onCreated(function () {
    this.autorun(()=> {
        this.subscribe('ppos.prepaidOrder', {_id: this.data._id});
    });
});
...
AutoForm.addHooks([
    'PPOS_prepaidOrderNew',
    'PPOS_prepaidOrderEdit'
], hooksObject);
