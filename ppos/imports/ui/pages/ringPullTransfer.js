...
// Declare template
let indexTmpl = Template.PPOS_ringPullTransfer,
    actionTmpl = Template.PPOS_ringPullTransferAction,
    newTmpl = Template.PPOS_ringPullTransferNew,
    editTmpl = Template.PPOS_ringPullTransferEdit,
    showTmpl = Template.PPOS_ringPullTransferShow
// Local collection
...
indexTmpl.events({
    'click .js-create' (event, instance) {
        alertify.ringPullTransfer(fa('cart-arrow-down', TAPi18n.__('ppos.ringPullTransfer.title')), renderTemplate(newTmpl)).maximize();
    },
    'click .js-update' (event, instance) {
        ...
        //     Meteor.call('ppos.isGroupRingPullTransferClosed', {_id: this.paymentGroupId}, (err, result)=> {
        //         if (result.paid) {
    ...
    'click .js-destroy' (event, instance) {
        ...
        destroyAction(
            RingPullTransfers,
            {_id: data._id},
            {title: TAPi18n.__('ppos.ringPullTransfer.title'), itemTitle: data._id}
        );
    },
    'click .js-display' (event, instance) {
        ...
        Meteor.call('ringPullTransferShowItems', {_id: this._id}, function (err, result) {
            swal.close();
            alertify.ringPullTransferShow(fa('eye', TAPi18n.__('ppos.ringPullTransfer.title')), renderTemplate(showTmpl, result)).maximize();
        });
    },
    'click .js-ringPullTransfer' (event, instance) {
        let params = {};
        let queryParams = {ringPullTransferId: this._id};
        let path = FlowRouter.path("ppos.ringPullTransferReportGen", params, queryParams);
...
function excuteEditForm(doc) {
    ...
    alertify.ringPullTransfer(fa('pencil', TAPi18n.__('ppos.ringPullTransfer.title')), renderTemplate(editTmpl, doc)).maximize();
}
// Hook
let hooksObject = {
    before: {
...
AutoForm.addHooks([
    'PPOS_ringPullTransferNew',
    'PPOS_ringPullTransferUpdate'
], hooksObject);
