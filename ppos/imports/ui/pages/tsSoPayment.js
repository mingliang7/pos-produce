...
import {TSSOPayment} from '../../api/collections/tsSoPayment';
...
import {tsSoPaymentFn} from '../../../common/methods/tsSoPayment';
//page
import './tsSoPayment.html';
import './penalty.html';
//methods
...
let indexTmpl = Template.PPOS_tsSoPayment,
    showInvoiceTmpl = Template.PPOS_invoiceShowInTsSoPayment;
    ...
    this.autorun(function () {
        if (Session.get('customerId')) {
            ...
            if (customer && customer.termId) {
                invoiceSub = Meteor.subscribe('ppos.activeTsSo', {
                    customerId: Session.get('customerId'),
                    $or: [{tsStatus: {$in: ['active', 'partial']}}, {tsStatus: {$exists: false}}],
                    totalTransportFee: {$ne: 0},
                });
            } else {
                invoiceSub = Meteor.subscribe('ppos.activeGroupSales', {
                    vendorOrCustomerId: Session.get('customerId'),
                    tsStatus: {$in: ['active', 'partial']}
                });
            }
            if (invoiceSub.ready()) {
                let invoices = customer.termId ? Order.find({}).fetch() : GroupInvoice.find({}).fetch();
        ...
        if (Session.get('invoices')) {
            Meteor.subscribe('ppos.tsSoPayment', {
                invoiceId: {
...
indexTmpl.helpers({
    ...
    countIsqualSales() {
        ...
        let collection = (customer && customer.termId) ? Order.find() : GroupInvoice.find();
    ...
    totalTransportFeeAmountDue(){
        ...
        let customer = getCustomerInfo(Session.get('customerId'));
        let invoices = (customer && customer.termId) ? Order.find({}) : GroupInvoice.find({});
        if (invoices.count() > 0) {
            invoices.forEach(function (invoice) {
                let tsSoPaments = TSSOPayment.find({invoiceId: invoice._id}, {sort: {_id: 1, paymentDate: 1}});
                if (tsSoPaments.count() > 0) {
                    let lastPayment = _.last(tsSoPaments.fetch());
...
indexTmpl.events({
    'click .showInvoice'(event, instance){
        ...
        Meteor.call('invoiceShowItems', {doc: this}, function (err, result) {
            swal.close();
            alertify.paymentShow(fa('eye', TAPi18n.__('ppos.invoice.title')), renderTemplate(showInvoiceTmpl, result)).maximize();
        });
    },
...
function getLastPayment(invoiceId) {
    let tsSoPaments = TSSOPayment.find({invoiceId: invoiceId}, {sort: {_id: 1, paymentDate: 1}});
    if (tsSoPaments.count() > 0) {
        let lastPayment = _.last(tsSoPaments.fetch());
