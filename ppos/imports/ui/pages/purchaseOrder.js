...
// Declare template
let indexTmpl = Template.PPOS_purchaseOrder,
    actionTmpl = Template.PPOS_purchaseOrderAction,
    newTmpl = Template.PPOS_purchaseOrderNew,
    editTmpl = Template.PPOS_purchaseOrderEdit,
    showTmpl = Template.PPOS_purchaseOrderShow;
...
indexTmpl.events({
    'click .js-create' (event, instance) {
        alertify.purchaseOrder(fa('plus', TAPi18n.__('ppos.purchaseOrder.title')), renderTemplate(newTmpl)).maximize();
    },
    'click .js-update' (event, instance) {
        let data = this;
        Meteor.call('isPurchaseOrderHasRelation', data._id, function (error, result) {
            if (error) {
                ...
                if (result) {
                    ...
                    alertify.purchaseOrder(fa('pencil', TAPi18n.__('ppos.purchaseOrder.title')), renderTemplate(editTmpl, data));
                }
            }
        });
    },
            ...
            if (error) {
                ...
                if (result) {
                    ...
                    destroyAction(
                        PurchaseOrder,
                        {_id: data._id},
                        {title: TAPi18n.__('ppos.purchaseOrder.title'), itemTitle: data._id}
                    );
                }
            }
        });
    ...
    'click .js-display' (event, instance) {
        ...
        Meteor.call('invoiceShowItems', {doc: this}, function (err, result) {
            swal.close();
            alertify.purchaseOrderShow(fa('eye', TAPi18n.__('ppos.invoice.title')), renderTemplate(showTmpl, result)).maximize();
        });
    },
    'click .js-invoice' (event, instance) {
        let params = {};
        let queryParams = {purchaseOrderId: this._id};
        let path = FlowRouter.path("ppos.purchaseOrderReportGen", params, queryParams);
...
editTmpl.onCreated(function () {
    this.autorun(()=> {
        this.subscribe('ppos.purchaseOrder', {_id: this.data._id});
    });
});
...
AutoForm.addHooks([
    'PPOS_purchaseOrderNew',
    'PPOS_purchaseOrderEdit'
], hooksObject);
