...
// Declare template
let indexTmpl = Template.PPOS_locationTransfer,
    actionTmpl = Template.PPOS_locationTransferAction,
    newTmpl = Template.PPOS_locationTransferNew,
    editTmpl = Template.PPOS_locationTransferEdit,
    showTmpl = Template.PPOS_locationTransferShow;
...
indexTmpl.events({
    'click .js-create' (event, instance) {
        alertify.locationTransfer(fa('plus', TAPi18n.__('ppos.locationTransfer.title')), renderTemplate(newTmpl)).maximize();
    },
    'click .js-update' (event, instance) {
        alertify.locationTransfer(fa('pencil', TAPi18n.__('ppos.locationTransfer.title')), renderTemplate(editTmpl, this));
    },
    'click .js-destroy' (event, instance) {
        ...
        destroyAction(
            LocationTransfers,
            {_id: data._id},
            {title: TAPi18n.__('ppos.locationTransfer.title'), itemTitle: data._id}
        );
    },
    'click .js-display' (event, instance) {
        Meteor.call('ppos.locationTransferInfo', {_id: this._id}, function (err, result) {
            if (result) {
                console.log(result);
    ...
    'click .js-locationTransfer' (event, instance) {
        ...
        let path = FlowRouter.path("ppos.locationTransferReportGen", params, queryParams);
...
editTmpl.onCreated(function () {
    this.autorun(()=> {
        this.subscribe('ppos.locationTransfer', {_id: this.data._id});
    });
});
editTmpl.events({
    'click #btn-save-print'(event, instance){
...
AutoForm.addHooks([
    'PPOS_locationTransferNew',
    'PPOS_locationTransferEdit'
], hooksObject);
