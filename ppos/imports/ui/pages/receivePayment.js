...
let indexTmpl = Template.PPOS_receivePayment,
    showInvoiceTmpl = Template.PPOS_invoiceShowInPayment;
...
indexTmpl.onCreated(function () {
    ...
    this.autorun(function () {
        Meteor.subscribe('ppos.advanceDiscount', {name: 'receivePayment'});
        if (Session.get('customerId')) {
            ...
            if (customer && customer.termId) {
                invoiceSub = Meteor.subscribe('ppos.activeInvoices', {
                    customerId: Session.get('customerId'),
                    status: {$in: ['active', 'partial']},
                    invoiceType: 'term'
                });
            } else {
                invoiceSub = Meteor.subscribe('ppos.activeGroupInvoices', {
        ...
        if (Session.get('invoices')) {
            Meteor.subscribe('ppos.receivePayment', {
                invoiceId: {
