...
// Declare template
let indexTmpl = Template.PPOS_closing,
    actionTmpl = Template.PPOS_closingAction,
    newTmpl = Template.PPOS_closingNew,
    editTmpl = Template.PPOS_closingEdit,
    showTmpl = Template.PPOS_closingShow;
...
indexTmpl.events({
    'click .js-create' (event, instance) {
        alertify.closing(fa('plus', TAPi18n.__('ppos.closing.title')), renderTemplate(newTmpl));
    },
            ...
            if (error) {
               ...
               if(!result){
                    destroyAction(
                        Closing,
                        {_id: id},
                        {title: TAPi18n.__('ppos.closing.title'), itemTitle: id}
                    );
               }else{
    ...
    'click .js-display' (event, instance) {
        alertify.closinghow(fa('eye', TAPi18n.__('ppos.closing.title')), renderTemplate(showTmpl, this));
    }
});
