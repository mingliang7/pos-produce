...
export const  orderSOBalanceReport = new ValidatedMethod({
    name: 'ppos.orderSOBalanceReport',
    ...
    run(params) {
        if (!this.isSimulation) {
            ...
            if (params.date) {
                ...
                if(params.type == 'all'){
                    selector.$or = [
                                        {paymentStatus: {$in: ['active', 'partial']}, orderDate: {$lte: date}},
                                        {orderDate: {$lte: date}, paymentStatus: 'closed', closedAt: {$gt: date}}
                                    ];
                }else{
                    selector.$or = [
            ...
            if(params.type == 'active'){
                ...
                orders = Order.aggregate([
                    ...
                    {
                        $lookup: {
                            from: "ppos_saleOrderReceivePayment",
                            localField: "_id",
